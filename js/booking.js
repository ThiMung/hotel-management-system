...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const roomPrices = { single: 600000, double: 1000000, family: 1600000, vip: 2800000 };
  ...
  function updateTotal() {
    ...
    selectedServicesList.innerHTML = selected.length
      ? selected.map((s, i) => `<li>${s.name} - ${s.price.toLocaleString()} VND <button type="button" onclick="removeService(${i})">x</button></li>`).join('')
  ...
  form.addEventListener('submit', e => {
    ...
    showMsg(`Booking confirmed! Total: ${serviceTotal.textContent} VND`, 'success');
